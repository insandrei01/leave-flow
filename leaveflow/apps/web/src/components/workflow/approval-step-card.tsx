...
/**
 * ApprovalStepCard — visual card for a single workflow approval step.
 *
 * Displays approver type selector (role-based / specific / group),
 * timeout configuration, escalation mode (timeout action), delegation
 * toggle, and move up/down/delete controls.
 *
 * All state changes are communicated via callbacks (immutable pattern).
 */
...
export function ApprovalStepCard({
  ...
  const accentColor = stepNumber % 2 === 0 ? "violet" : "indigo";
  const borderColor =
    accentColor === "violet"
  ...
  return (
    <div
      className={cn("glass-card border-l-4 p-5 transition-all", borderColor)}
      role="group"
      aria-label={`Approval step ${stepNumber}`}
    >
      {/* Header */}
          ...
          <div className="flex gap-2">
            {(["role-based", "specific", "group"] as const).map((type) => (
              <button
                ...
                aria-pressed={step.approverType === type}
                className={cn(
        ...
        <div className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 px-4 py-3">
          ...
          <button
            ...
            className={cn(
              ...
              step.allowDelegation ? "bg-accent-indigo" : "bg-white/10"
            )}
          >
            <span
              className={cn(
