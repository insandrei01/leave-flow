/**
 * Onboarding route-level Zod schemas (steps 1-6).
 *
 * These mirror the schemas in packages/validation/src/onboarding.schema.ts.
...
export const onboardingStep3Schema = z
  ...
  .refine(
    (body) =>
      ...
      (body.customSteps !== null &&
        body.customSteps !== undefined &&
        body.customSteps.length > 0),
...
const teamEntrySchema = z.object({
  ...
  workflowId: mongoIdSchema.nullish().transform((v) => v ?? null),
});
...
const employeeEntrySchema = z.object({
  ...
  teamId: mongoIdSchema.nullish().transform((v) => v ?? null),
...
/**
 * Path params for PUT /onboarding/steps/:stepNumber
 */
export const stepParamsSchema = z.object({
  stepNumber: z.coerce
    .number()
    .int("Step number must be an integer")
    .min(1, "Step number must be between 1 and 6")
    .max(6, "Step number must be between 1 and 6"),
});
